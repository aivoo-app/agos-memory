...
pub fn run_remember(
    ...
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|e| Error::Storage(format!("cannot start async runtime: {e}")))?;
    rt.block_on(async {
...
pub fn run_session(cfg: &Config, cmd: &super::root::SessionCmd) -> Result<()> {
            ...
            SessionCmd::Append {
                ...
                let id = if public_id.is_empty() {
                    ...
                    sessions::session_id_by_public(&store, &public_id)
                        .await?
                        .ok_or_else(|| {
                            Error::InvalidInput(format!("unknown session {public_id}"))
                        })?
                };
